...
pub fn writeTransmitRgbaWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, image_id: u32, rgba: []const u8, w: i32, h: i32) !void {
...
/// of a regular file.
pub fn writeTransmitRgbaFileWhole(out: std.fs.File.DeprecatedWriter, image_id: u32, path: []const u8, w: i32, h: i32) !void {
    try writeTransmitRgbaFileWholeWithQuiet(out, .suppress_fail, image_id, path, w, h);
}

pub fn writeTransmitRgbaFileWholeWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, image_id: u32, path: []const u8, w: i32, h: i32) !void {
...
pub fn writeTransmitRgbaFileRegion(out: std.fs.File.DeprecatedWriter, image_id: u32, path: []const u8, file_offset: u64, byte_len: usize, w: i32, h: i32) !void {
    try writeTransmitRgbaFileRegionWithQuiet(out, .suppress_fail, image_id, path, file_offset, byte_len, w, h);
}

pub fn writeTransmitRgbaFileRegionWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, image_id: u32, path: []const u8, file_offset: u64, byte_len: usize, w: i32, h: i32) !void {
...
pub fn writeQueryFileRgbaWhole(out: std.fs.File.DeprecatedWriter, path: []const u8, w: i32, h: i32) !void {
    var prefix_buf: [160]u8 = undefined;
    const prefix = try std.fmt.bufPrint(&prefix_buf, "i=32,a=q,t=f,f=32,s={d},v={d}", .{ w, h });
    try writeEncodedPayloadApc(out, prefix, path);
}

pub fn writeQueryFileRgbaRegion(out: std.fs.File.DeprecatedWriter, path: []const u8, file_offset: u64, byte_len: usize, w: i32, h: i32) !void {
    var prefix_buf: [192]u8 = undefined;
...
pub fn writePlaceWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, row: i32, col: i32, placement: Placement) !void {
...
pub fn writeDeleteExactPlacementWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, target: ExactPlacement) !void {
    try out.print("\x1b_Gq={d},a=d,d=i,i={d},p={d};\x1b\\", .{ @intFromEnum(quiet), target.image_id, target.placement_id });
}

pub fn writeDeleteImageWithQuiet(out: std.fs.File.DeprecatedWriter, quiet: Quiet, mode: ImageDeleteMode, image_id: u32) !void {
...
fn writeEncodedPayloadApc(out: std.fs.File.DeprecatedWriter, prefix: []const u8, payload: []const u8) !void {
    const enc_len = std.base64.standard.Encoder.calcSize(payload.len);
    const b64 = try std.heap.page_allocator.alloc(u8, enc_len);
    defer std.heap.page_allocator.free(b64);
    _ = std.base64.standard.Encoder.encode(b64, payload);
