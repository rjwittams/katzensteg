/* Real SDL2 functions Katzensteg calls, as an X-macro list. Each resolution
 * backend defines KS_REAL(name, rettype, args, params) and
 * KS_REAL_VOID(name, args, params), includes this file, and so provides one
 * ks_real_<name> per entry: dlsym(RTLD_NEXT) in real_sdl_linux.c and the
 * saved SDL_DYNAMIC_API jump table in real_sdl_dynapi.c. */
...
KS_REAL(SDL_GL_GetProcAddress, void *, (const char *proc), (proc))
...
KS_REAL(SDL_WaitEventTimeout, int, (union SDL_Event *event, int timeout), (event, timeout))
KS_REAL(SDL_GetRendererOutputSize, int, (struct SDL_Renderer *renderer, int *w, int *h), (renderer, w, h))
KS_REAL(SDL_ClearError, void, (void), ())
...
KS_REAL(SDL_GetModState, int, (void), ())
KS_REAL(SDL_GetKeyFromScancode, int, (int scancode), (scancode))
KS_REAL(SDL_GetScancodeFromKey, int, (int key), (key))
