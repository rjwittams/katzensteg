/* OpenGL functions for GL framebuffer capture, resolved through the loading
 * SDL's SDL_GL_GetProcAddress. SDL_DYNAMIC_API builds use this instead of
 * dlsym: SDL loads the GL library privately, and on Windows extension and
 * core functions come from wglGetProcAddress. Capture runs in the
 * SDL_GL_SwapWindow wrapper with the application's context current, which
 * SDL_GL_GetProcAddress requires. */
...
#if defined(KS_GL_VIA_LINKED_SDL)
/* Unit tests link SDL directly instead of wrapping a loading SDL. */
extern void *SDL_GL_GetProcAddress(const char *proc);
#define ks_real_SDL_GL_GetProcAddress SDL_GL_GetProcAddress
#else
extern void *ks_real_SDL_GL_GetProcAddress(const char *proc);
#endif

static void *ks_gl_symbol(const char *name) {
    return ks_real_SDL_GL_GetProcAddress(name);
}
...
static void *ks_required_gl_symbol(const char *name) {
    ...
    if (!symbol) {
        char message[256];
        snprintf(message, sizeof(message), "SDL_GL_GetProcAddress found no %s", name);
