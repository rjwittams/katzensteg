...
const sdl_adapter = @import("sdl3_adapter.zig");
const real_gl = @import("real_gl.zig");
const real_sdl = @import("real_sdl3.zig");
const sdl_input = @import("sdl3_input_adapter.zig");
...
fn windowEventTraceName(event: sdl.Uint32) []const u8 {
    return switch (event) {
...
fn sdlEventTraceName(event: *const sdl.SDL_Event) []const u8 {
    return switch (event.type) {
        ...
        sdl.SDL_WINDOWEVENT_FOCUS_LOST,
        sdl.SDL_WINDOWEVENT_CLOSE,
        => windowEventTraceName(event.type),
...
fn traceSdlEvent(comptime source: []const u8, event: *const sdl.SDL_Event, suppressed: bool) void {
    ...
    if (event.type >= sdl.SDL_WINDOWEVENT_RESIZED and event.type <= sdl.SDL_WINDOWEVENT_CLOSE) {
        sdl_log.debug(
            "{s} {s} type=0x{x} window={d} data={d},{d} suppressed={} tracked_windows={d}",
            .{
                source,
        ...
        sdl_log.debug(
            "{s} {s} type=0x{x} window={d} which={d} pos={d:.1},{d:.1} rel={d:.1},{d:.1} buttons=0x{x} suppressed={} tracked_windows={d}",
            .{
                source,
                sdlEventTraceName(event),
                event.type,
        ...
        sdl_log.debug(
            "{s} {s} type=0x{x} window={d} which={d} button={d} down={} clicks={d} pos={d:.1},{d:.1} suppressed={} tracked_windows={d}",
            .{
                source,
                sdlEventTraceName(event),
                event.type,
        ...
        sdl_log.debug(
            "{s} {s} type=0x{x} window={d} which={d} wheel={d:.1},{d:.1} mouse={d:.1},{d:.1} suppressed={} tracked_windows={d}",
            .{
                source,
                sdlEventTraceName(event),
                event.type,
...
fn applyRealWindowAction(action: window_policy.RealWindowAction, window: ?*sdl.SDL_Window) bool {
...
pub export fn ks_SDL_Init(flags: sdl.Uint32) callconv(.c) sdl.SDL_bool {
    applyBackgroundGamepadHint();
    return real_sdl.SDL_Init(flags);
}

pub export fn ks_SDL_InitSubSystem(flags: sdl.Uint32) callconv(.c) sdl.SDL_bool {
    applyBackgroundGamepadHint();
...
pub export fn ks_SDL_SetHint(name: [*:0]const u8, value: [*:0]const u8) callconv(.c) sdl.SDL_bool {
    ...
    if (shouldTraceSdlHint(name_slice)) {
        traceSdlLifecycle("SDL_SetHint {s}={s} rc={}", .{ name_slice, std.mem.span(value), rc });
    }
    return rc;
}
...
pub export fn ks_SDL_CreateWindow(title: [*:0]const u8, w: c_int, h: c_int, flags: sdl.SDL_WindowFlags) callconv(.c) ?*sdl.SDL_Window {
    const window = real_sdl.SDL_CreateWindow(title, w, h, flags);
    ...
    _ = applyRealWindowAction(rt.realWindowCreateAction(), window);
    return window;
}

pub export fn ks_SDL_SetWindowSize(window: ?*sdl.SDL_Window, w: c_int, h: c_int) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = real_sdl.SDL_SetWindowSize(window, w, h);
    traceLimited(rt, &trace_set_window_size, "SDL_SetWindowSize window={x} size={d}x{d} rc={}", .{ if (window) |p| @intFromPtr(p) else 0, w, h, rc });
    ...
    if (rc) dispatchWindowSizeByHandle(rt, sdl_adapter.handleFromPtr(window), w, h);
    return rc;
}

pub export fn ks_SDL_GetWindowFlags(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_WindowFlags {
    const flags = real_sdl.SDL_GetWindowFlags(window);
    return sdl_input.claimedWindowFlags(runtime.get(), flags);
}

pub export fn ks_SDL_ShowWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    ...
    return applyRealWindowAction(action, window);
}

pub export fn ks_SDL_HideWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    ...
    if (action == .none) {
        return real_sdl.SDL_HideWindow(window);
    } else {
        return applyRealWindowAction(action, window);
    }
}

pub export fn ks_SDL_MinimizeWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const action = rt.realWindowCreateAction();
    ...
    if (action == .none) {
        return real_sdl.SDL_MinimizeWindow(window);
    } else {
        return applyRealWindowAction(action, window);
    }
}

pub export fn ks_SDL_RestoreWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
...
pub export fn ks_SDL_RaiseWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const action = rt.realWindowShowAction();
    traceLimited(rt, &trace_raise_window, "SDL_RaiseWindow window={x} action={s}", .{ if (window) |p| @intFromPtr(p) else 0, @tagName(action) });
    if (action == .show) {
        return real_sdl.SDL_RaiseWindow(window);
    } else {
        return applyRealWindowAction(action, window);
    }
}
...
pub export fn ks_SDL_CreateRenderer(window: ?*sdl.SDL_Window, name: ?[*:0]const u8) callconv(.c) ?*sdl.SDL_Renderer {
    const renderer = real_sdl.SDL_CreateRenderer(window, name);
    const rt = runtime.get();
    traceLimited(rt, &trace_create_renderer, "SDL_CreateRenderer window={x} renderer={x} name={s}", .{ if (window) |p| @intFromPtr(p) else 0, if (renderer) |p| @intFromPtr(p) else 0, if (name) |z| std.mem.span(z) else "(null)" });
    switch (rt.intercept_mode) {
...
pub export fn ks_SDL_UpdateTexture(texture: ?*sdl.SDL_Texture, rect: ?*const sdl.SDL_Rect, pixels: ?*const anyopaque, pitch: c_int) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_UpdateTexture(texture, rect, pixels, pitch);
    const rt = runtime.get();
    traceLimited(rt, &trace_update_texture, "SDL_UpdateTexture texture={x} rc={} rect={s} pitch={d} pixels={x}", .{ if (texture) |p| @intFromPtr(p) else 0, rc, if (rect == null) "null" else "partial", pitch, if (pixels) |p| @intFromPtr(p) else 0 });
    if (rc) switch (rt.intercept_mode) {
...
pub export fn ks_SDL_UpdateYUVTexture(texture: ?*sdl.SDL_Texture, rect: ?*const sdl.SDL_Rect, yplane: ?[*]const sdl.Uint8, ypitch: c_int, uplane: ?[*]const sdl.Uint8, upitch: c_int, vplane: ?[*]const sdl.Uint8, vpitch: c_int) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_UpdateYUVTexture(texture, rect, yplane, ypitch, uplane, upitch, vplane, vpitch);
    const rt = runtime.get();
    traceLimited(rt, &trace_update_yuv_texture, "SDL_UpdateYUVTexture texture={x} rc={} rect={s} ypitch={d} upitch={d} vpitch={d}", .{ if (texture) |p| @intFromPtr(p) else 0, rc, if (rect == null) "null" else "partial", ypitch, upitch, vpitch });
    if (rc) switch (rt.intercept_mode) {
...
pub export fn ks_SDL_UpdateNVTexture(texture: ?*sdl.SDL_Texture, rect: ?*const sdl.SDL_Rect, yplane: ?[*]const sdl.Uint8, ypitch: c_int, uvplane: ?[*]const sdl.Uint8, uvpitch: c_int) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_UpdateNVTexture(texture, rect, yplane, ypitch, uvplane, uvpitch);
    const rt = runtime.get();
    traceLimited(rt, &trace_update_nv_texture, "SDL_UpdateNVTexture texture={x} rc={} rect={s} ypitch={d} uvpitch={d}", .{ if (texture) |p| @intFromPtr(p) else 0, rc, if (rect == null) "null" else "partial", ypitch, uvpitch });
    if (rc) switch (rt.intercept_mode) {
...
pub export fn ks_SDL_LockTexture(texture: ?*sdl.SDL_Texture, rect: ?*const sdl.SDL_Rect, pixels: *?*anyopaque, pitch: *c_int) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_LockTexture(texture, rect, pixels, pitch);
    const rt = runtime.get();
    traceLimited(rt, &trace_lock_texture, "SDL_LockTexture texture={x} rc={} rect={s} pitch={d} pixels={x}", .{ if (texture) |p| @intFromPtr(p) else 0, rc, if (rect == null) "null" else "partial", if (rc) pitch.* else 0, if (rc) if (pixels.*) |p| @intFromPtr(p) else 0 else 0 });
    if (rc) {
        switch (rt.intercept_mode) {
...
pub export fn ks_SDL_SetTextureColorMod(texture: ?*sdl.SDL_Texture, r: sdl.Uint8, g: sdl.Uint8, b: sdl.Uint8) callconv(.c) sdl.SDL_bool {
...
pub export fn ks_SDL_SetTextureAlphaMod(texture: ?*sdl.SDL_Texture, a: sdl.Uint8) callconv(.c) sdl.SDL_bool {
...
pub export fn ks_SDL_SetTextureBlendMode(texture: ?*sdl.SDL_Texture, blendMode: sdl.Uint32) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_SetTextureBlendMode(texture, blendMode);
    if (rc) {
        const rt = runtime.get();
        switch (rt.intercept_mode) {
            .sync_compose => sink.onSetTextureBlendMode(rt, texture, @intCast(blendMode)),
            .queued_replay => sink.dispatchCommand(rt, .{ .set_texture_blend_mode = .{ .texture = sdl_adapter.handleFromPtr(texture), .blend_mode = sdl_adapter.blendModeFromSdl2(@intCast(blendMode)) } }),
        }
    }
    return rc;
}

pub export fn ks_SDL_SetRenderDrawColor(renderer: ?*sdl.SDL_Renderer, r: sdl.Uint8, g: sdl.Uint8, b: sdl.Uint8, a: sdl.Uint8) callconv(.c) sdl.SDL_bool {
...
pub export fn ks_SDL_RenderClear(renderer: ?*sdl.SDL_Renderer) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderClear(renderer) else true;
    if (rc) {
...
fn rectToFRect(rect: ?*const sdl.SDL_Rect) ?sdl.SDL_FRect {
    const r = rect orelse return null;
    return .{ .x = @floatFromInt(r.x), .y = @floatFromInt(r.y), .w = @floatFromInt(r.w), .h = @floatFromInt(r.h) };
}

fn pointToFPoint(point: ?*const sdl.SDL_Point) ?sdl.SDL_FPoint {
    const p = point orelse return null;
    return .{ .x = @floatFromInt(p.x), .y = @floatFromInt(p.y) };
}

pub export fn ks_SDL_RenderTexture(renderer: ?*sdl.SDL_Renderer, texture: ?*sdl.SDL_Texture, srcrect: ?*const sdl.SDL_FRect, dstrect: ?*const sdl.SDL_FRect) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderTexture(renderer, texture, srcrect, dstrect) else true;
    if (rc) {
        traceLimited(rt, &trace_render_copy, "SDL_RenderTexture renderer={x} texture={x} src={s} dst={s}", .{ if (renderer) |p| @intFromPtr(p) else 0, if (texture) |p| @intFromPtr(p) else 0, if (srcrect == null) "null" else "set", if (dstrect == null) "null" else "set" });
        const src_rect = if (srcrect) |r| sdl.SDL_Rect{ .x = @intFromFloat(r.x), .y = @intFromFloat(r.y), .w = @intFromFloat(r.w), .h = @intFromFloat(r.h) } else null;
        const dst_rect = if (dstrect) |r| sdl.SDL_Rect{ .x = @intFromFloat(r.x), .y = @intFromFloat(r.y), .w = @intFromFloat(r.w), .h = @intFromFloat(r.h) } else null;
        if (rt.captureEnabled()) {
            switch (rt.intercept_mode) {
                .sync_compose => sink.onRenderCopy(rt, renderer, texture, if (src_rect) |*r| r else null, if (dst_rect) |*r| r else null),
                .queued_replay => sink.dispatchCommand(rt, .{ .render_copy = .{ .renderer = sdl_adapter.handleFromPtr(renderer), .texture = sdl_adapter.handleFromPtr(texture), .src = sdl_adapter.rectFromSdl(if (src_rect) |*r| r else null), .dst = sdl_adapter.rectFromSdl(if (dst_rect) |*r| r else null) } }),
            }
        }
    }
    return rc;
...
pub export fn ks_SDL_RenderTextureRotated(renderer: ?*sdl.SDL_Renderer, texture: ?*sdl.SDL_Texture, srcrect: ?*const sdl.SDL_FRect, dstrect: ?*const sdl.SDL_FRect, angle: f64, center: ?*const sdl.SDL_FPoint, flip: c_int) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderTextureRotated(renderer, texture, srcrect, dstrect, angle, center, flip) else true;
    if (rc) {
        traceLimited(rt, &trace_render_copy_ex, "SDL_RenderTextureRotated renderer={x} texture={x} src={s} dst={s} angle={d:.2} flip={d}", .{ if (renderer) |p| @intFromPtr(p) else 0, if (texture) |p| @intFromPtr(p) else 0, if (srcrect == null) "null" else "set", if (dstrect == null) "null" else "set", angle, flip });
        const src_rect = if (srcrect) |r| sdl.SDL_Rect{ .x = @intFromFloat(r.x), .y = @intFromFloat(r.y), .w = @intFromFloat(r.w), .h = @intFromFloat(r.h) } else null;
        const dst_rect = if (dstrect) |r| sdl.SDL_Rect{ .x = @intFromFloat(r.x), .y = @intFromFloat(r.y), .w = @intFromFloat(r.w), .h = @intFromFloat(r.h) } else null;
        const center_i = if (center) |p| sdl.SDL_Point{ .x = @intFromFloat(p.x), .y = @intFromFloat(p.y) } else null;
        if (rt.captureEnabled()) {
            switch (rt.intercept_mode) {
                .sync_compose => sink.onRenderCopyEx(rt, renderer, texture, if (src_rect) |*r| r else null, if (dst_rect) |*r| r else null, angle, if (center_i) |*p| p else null, flip),
                .queued_replay => sink.dispatchCommand(rt, .{ .render_copy_ex = .{ .renderer = sdl_adapter.handleFromPtr(renderer), .texture = sdl_adapter.handleFromPtr(texture), .src = sdl_adapter.rectFromSdl(if (src_rect) |*r| r else null), .dst = sdl_adapter.rectFromSdl(if (dst_rect) |*r| r else null), .angle = angle, .center = sdl_adapter.pointFromSdl(if (center_i) |*p| p else null), .flip = flip } }),
            }
        }
    }
    return rc;
...
pub export fn ks_SDL_RenderGeometryRaw(renderer: ?*sdl.SDL_Renderer, texture: ?*sdl.SDL_Texture, xy: ?[*]const f32, xy_stride: c_int, color: ?[*]const sdl.SDL_FColor, color_stride: c_int, uv: ?[*]const f32, uv_stride: c_int, num_vertices: c_int, indices: ?*const anyopaque, num_indices: c_int, size_indices: c_int) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderGeometryRaw(renderer, texture, xy, xy_stride, color, color_stride, uv, uv_stride, num_vertices, indices, num_indices, size_indices) else true;
    if (rc) {
...
pub export fn ks_SDL_RenderFillRect(renderer: ?*sdl.SDL_Renderer, rect: ?*const sdl.SDL_FRect) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderFillRect(renderer, rect) else true;
    if (rc) {
        const rect_i = if (rect) |r| sdl.SDL_Rect{ .x = @intFromFloat(r.x), .y = @intFromFloat(r.y), .w = @intFromFloat(r.w), .h = @intFromFloat(r.h) } else null;
        if (rt.captureEnabled()) {
            switch (rt.intercept_mode) {
                .sync_compose => sink.onRenderFillRect(rt, renderer, if (rect_i) |*r| r else null),
                .queued_replay => sink.dispatchCommand(rt, .{ .render_fill_rect = .{ .renderer = sdl_adapter.handleFromPtr(renderer), .rect = sdl_adapter.rectFromSdl(if (rect_i) |*r| r else null) } }),
            }
        }
    }
    return rc;
...
pub export fn ks_SDL_RenderPoint(renderer: ?*sdl.SDL_Renderer, x: f32, y: f32) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderPoint(renderer, x, y) else true;
    if (rc) {
        if (rt.captureEnabled()) {
            const x_i: c_int = @intFromFloat(x);
            const y_i: c_int = @intFromFloat(y);
            switch (rt.intercept_mode) {
                .sync_compose => sink.onRenderDrawPoint(rt, renderer, x_i, y_i),
                .queued_replay => sink.dispatchCommand(rt, .{ .render_draw_point = .{ .renderer = sdl_adapter.handleFromPtr(renderer), .x = x_i, .y = y_i } }),
            }
        }
    }
    return rc;
...
pub export fn ks_SDL_RenderLine(renderer: ?*sdl.SDL_Renderer, x1: f32, y1: f32, x2: f32, y2: f32) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    const rc = if (rt.realRenderEnabled()) real_sdl.SDL_RenderLine(renderer, x1, y1, x2, y2) else true;
    if (rc) {
        if (rt.captureEnabled()) {
            const x1_i: c_int = @intFromFloat(x1);
            const y1_i: c_int = @intFromFloat(y1);
            const x2_i: c_int = @intFromFloat(x2);
            const y2_i: c_int = @intFromFloat(y2);
            switch (rt.intercept_mode) {
                .sync_compose => sink.onRenderDrawLine(rt, renderer, x1_i, y1_i, x2_i, y2_i),
                .queued_replay => sink.dispatchCommand(rt, .{ .render_draw_line = .{ .renderer = sdl_adapter.handleFromPtr(renderer), .x1 = x1_i, .y1 = y1_i, .x2 = x2_i, .y2 = y2_i } }),
            }
        }
    }
    return rc;
...
pub export fn ks_SDL_SetRenderViewport(renderer: ?*sdl.SDL_Renderer, rect: ?*const sdl.SDL_Rect) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_SetRenderViewport(renderer, rect);
    if (rc) {
        const rt = runtime.get();
        switch (rt.intercept_mode) {
...
pub export fn ks_SDL_SetRenderClipRect(renderer: ?*sdl.SDL_Renderer, rect: ?*const sdl.SDL_Rect) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_SetRenderClipRect(renderer, rect);
    if (rc) {
        const rt = runtime.get();
        switch (rt.intercept_mode) {
...
pub export fn ks_SDL_RenderPresent(renderer: ?*sdl.SDL_Renderer) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    ...
    if (rt.realRenderEnabled()) return real_sdl.SDL_RenderPresent(renderer);
    return true;
}
...
pub export fn ks_SDL_GL_MakeCurrent(window: ?*sdl.SDL_Window, context: sdl.SDL_GLContext) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_GL_MakeCurrent(window, context);
    const rt = runtime.get();
    traceLimited(rt, &trace_gl_make_current, "SDL_GL_MakeCurrent window={x} context={x} rc={}", .{ if (window) |p| @intFromPtr(p) else 0, if (context) |p| @intFromPtr(p) else 0, rc });
    return rc;
}

pub export fn ks_SDL_GL_SwapWindow(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
    ...
    return real_sdl.SDL_GL_SwapWindow(window);
}

pub export fn ks_SDL_Vulkan_LoadLibrary(path: ?[*:0]const u8) callconv(.c) sdl.SDL_bool {
...
pub export fn ks_SDL_PollEvent(event: ?*sdl.SDL_Event) callconv(.c) sdl.SDL_bool {
    const rt = runtime.get();
...
pub export fn ks_SDL_SetEventEnabled(event_type: sdl.Uint32, enabled: sdl.SDL_bool) callconv(.c) void {
    real_sdl.SDL_SetEventEnabled(event_type, enabled);
}

pub export fn ks_SDL_EventEnabled(event_type: sdl.Uint32) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_EventEnabled(event_type);
}

pub export fn ks_SDL_StartTextInput(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_StartTextInput(window);
}

pub export fn ks_SDL_StopTextInput(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_StopTextInput(window);
}

pub export fn ks_SDL_TextInputActive(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_TextInputActive(window);
}

pub export fn ks_SDL_SetTextInputArea(window: ?*sdl.SDL_Window, rect: ?*const sdl.SDL_Rect, cursor: c_int) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_SetTextInputArea(window, rect, cursor);
}

pub export fn ks_SDL_GetTextInputArea(window: ?*sdl.SDL_Window, rect: ?*sdl.SDL_Rect, cursor: ?*c_int) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_GetTextInputArea(window, rect, cursor);
}

pub export fn ks_SDL_HasKeyboard() callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_HasKeyboard();
}
...
pub export fn ks_SDL_GetKeyboardState(numkeys: ?*c_int) callconv(.c) ?[*]const sdl.SDL_bool {
...
pub export fn ks_SDL_GetModState() callconv(.c) sdl.SDL_Keymod {
    const rt = runtime.get();
    if (!rt.input_enabled) return real_sdl.SDL_GetModState();
    sdl_input.refreshInput(rt);
    return sdl_input.mergedModifiers(rt, real_sdl.SDL_GetModState());
}

pub export fn ks_SDL_SetModState(modstate: sdl.SDL_Keymod) callconv(.c) void {
    real_sdl.SDL_SetModState(modstate);
}
...
pub export fn ks_SDL_GetMouseState(x: ?*f32, y: ?*f32) callconv(.c) sdl.Uint32 {
    ...
    if (realMouseFocused() and !rt.hasRemoteInput() and !rt.inputFocusSuspended()) {
        ...
        rt.dispatchCursorPosition(.{ .x = @intFromFloat(out_x.*), .y = @intFromFloat(out_y.*) });
        traceLimited(rt, &trace_get_mouse_state, "SDL_GetMouseState real pos={d:.1},{d:.1} buttons=0x{x}", .{ out_x.*, out_y.*, buttons });
        return rt.filterNativeMouseButtons(buttons) | rt.remoteMouseButtons();
    }
    if (rt.terminalMouseState()) |state| {
        if (x) |out_x| out_x.* = state.precise_x orelse @floatFromInt(state.x);
        if (y) |out_y| out_y.* = state.precise_y orelse @floatFromInt(state.y);
...
pub export fn ks_SDL_GetGlobalMouseState(x: ?*f32, y: ?*f32) callconv(.c) sdl.Uint32 {
    return real_sdl.SDL_GetGlobalMouseState(x, y);
}
...
pub export fn ks_SDL_GetRelativeMouseState(x: ?*f32, y: ?*f32) callconv(.c) sdl.Uint32 {
    ...
    if (rt.terminalRelativeMouseState()) |state| {
        if (x) |out_x| out_x.* = state.precise_xrel orelse @floatFromInt(state.xrel);
        if (y) |out_y| out_y.* = state.precise_yrel orelse @floatFromInt(state.yrel);
        return state.buttons;
    }
    const buttons = real_sdl.SDL_GetRelativeMouseState(x, y);
    const xrel = if (x) |out_x| out_x.* else 0.0;
    const yrel = if (y) |out_y| out_y.* else 0.0;
    if (buttons != 0 or xrel != 0 or yrel != 0) rt.claimRealWindowMouse();
    return rt.filterNativeMouseButtons(buttons) | rt.remoteMouseButtons();
}

pub export fn ks_SDL_SetWindowRelativeMouseMode(window: ?*sdl.SDL_Window, enabled: sdl.SDL_bool) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_SetWindowRelativeMouseMode(window, enabled);
}

pub export fn ks_SDL_GetWindowRelativeMouseMode(window: ?*sdl.SDL_Window) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_GetWindowRelativeMouseMode(window);
}

pub export fn ks_SDL_CaptureMouse(enabled: sdl.SDL_bool) callconv(.c) sdl.SDL_bool {
    return real_sdl.SDL_CaptureMouse(enabled);
}
...
pub export fn ks_SDL_SetCursor(cursor: ?*sdl.SDL_Cursor) callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_SetCursor(cursor);
    const rt = runtime.get();
    const handle = if (cursor) |p| @intFromPtr(p) else 0;
...
pub export fn ks_SDL_ShowCursor() callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_ShowCursor();
    if (rc) {
        const rt = runtime.get();
        sink.dispatchCommand(rt, .{ .show_cursor = .{ .visible = true } });
    }
    traceSdlLifecycle("SDL_ShowCursor rc={}", .{rc});
    return rc;
}

pub export fn ks_SDL_HideCursor() callconv(.c) sdl.SDL_bool {
    const rc = real_sdl.SDL_HideCursor();
    if (rc) {
        const rt = runtime.get();
        sink.dispatchCommand(rt, .{ .show_cursor = .{ .visible = false } });
    }
    traceSdlLifecycle("SDL_HideCursor rc={}", .{rc});
    return rc;
}

pub export fn ks_SDL_DestroyCursor(cursor: ?*sdl.SDL_Cursor) callconv(.c) void {
    const rt = runtime.get();
    const handle = if (cursor) |p| @intFromPtr(p) else 0;
    sink.dispatchCommand(rt, .{ .free_cursor = .{ .cursor = handle } });
    real_sdl.SDL_DestroyCursor(cursor);
    traceSdlLifecycle("SDL_DestroyCursor cursor={x}", .{handle});
}
...
test "SDL exit trace helpers name quit and claimed-window events" {
    ...
    close_event.window = .{
        .type = sdl.SDL_WINDOWEVENT_CLOSE,
        .reserved = 0,
        .timestamp = 0,
        .windowID = 7,
        .data1 = 0,
