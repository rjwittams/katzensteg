/* Real SDL3 functions Katzensteg calls, as an X-macro list. Each resolution
 * backend defines KS_REAL(name, rettype, args, params) and
 * KS_REAL_VOID(name, args, params), includes this file, and so provides one
 * ks_real_<name> per entry: dlsym(RTLD_NEXT) in real_sdl3_linux.c and the
 * saved SDL3_DYNAMIC_API jump table in dynapi_sdl3.c. */
