...
int run(DWORD flags){
  ...
  HPCON pc; HRESULT hr=CreatePseudoConsole((COORD){80,25},inR,outW,flags,&pc);
  ...
  WaitForSingleObject(pi.hProcess,5000); Sleep(300); ClosePseudoConsole(pc);
  char buf[65536]; DWORD n,tot=0; while(ReadFile(outR,buf+tot,sizeof(buf)-tot,&n,NULL)&&n){tot+=n;}
  dump(buf,tot); printf("\n"); return 0;}
int main(int argc,char**argv){ if(argc>1&&!strcmp(argv[1],"child")) return child(); run(0); run(0x8); return 0;}
