...
The runtime then acknowledges the completed presentation detach:

```json
{"type":"detached","window_id":"main"}
```

If the host wants to dispose the whole producer session, it should send `shutdown`:

```json
{"type":"shutdown"}
```

`shutdown` is session-scoped, so it does not carry a `window_id`. The runtime treats it as a detach for `main` and emits the same cleanup/`detached` sequence. The launcher also observes this control line and, after a short grace period for cleanup output to drain, terminates the target process group if the target is still running. Closing launcher stdin is also treated as host disappearance and triggers the same supervised termination path, though an explicit `shutdown` gives the runtime the best chance to emit cleanup first. Hosts should keep reading stdout after sending `detach` or `shutdown`; cleanup frames and lifecycle acknowledgements are part of the protocol output.
...
These examples are illustrative, not a frozen schema. In the current implementation, `attach`, `viewport`, `detach`, `shutdown`, `frame_batch`, and `detached` are the reliable messages. `hello`, `launched`, `window_created`, and `status` are reserved or optional lifecycle shape, not required for a minimal host. The important first-cut commitments are JSONL, explicit launcher embed mode, `window_id`, host attach before graphics output, host-provided id ranges, host-provided geometry/upload policy, explicit cleanup acknowledgement, and opaque renderer-generated byte groups.
