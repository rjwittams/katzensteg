...
CORE_PATH="${CORE_PATH:-$HOME/Library/Application Support/RetroArch/cores/melonds_libretro.dylib}"
CONFIG_PATH="${CONFIG_PATH:-/tmp/retroarch-sdl2.cfg}"
ROM_PATH="${ROM_PATH:-$HOME/roms/Super Mario 64 DS (Europe) (En,Fr,De,Es,It).nds}"
KATZENSTEG_LIB="${KATZENSTEG_LIB:-$ROOT/zig-out/lib/libkatzensteg-unlinked.dylib}"
OUTPUT_LOG="${OUTPUT_LOG:-/tmp/retro-sm64ds-melonds.out}"
...
env_args=(
  ...
  KATZENSTEG_GL_CAPTURE="${KATZENSTEG_GL_CAPTURE:-pbo}"
...
echo "Running RetroArch + melonDS + Super Mario 64 DS with Katzensteg"
...
echo "  KATZENSTEG_GL_CAPTURE=${KATZENSTEG_GL_CAPTURE:-pbo}"
echo "  KATZENSTEG_INPUT=${KATZENSTEG_INPUT:-1}"
echo "  KATZENSTEG_INPUT_CLAIM=${KATZENSTEG_INPUT_CLAIM:-1}"
echo "  OUTPUT_LOG=$OUTPUT_LOG"
if [[ -n "${KATZENSTEG_REAL_WINDOW:-}" ]]; then
  echo "  KATZENSTEG_REAL_WINDOW=$KATZENSTEG_REAL_WINDOW"
fi
if is_enabled "${KATZENSTEG_COMPOSITE_DEBUG:-}"; then
