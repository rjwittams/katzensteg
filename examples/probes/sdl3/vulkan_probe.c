...
static bool init_app(App *app, const Options *options)
{
    if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS)) {
        fprintf(stderr, "katzensteg-vulkan-probe-sdl3: SDL_Init failed: %s\n", SDL_GetError());
        return false;
    }
    if (!SDL_Vulkan_LoadLibrary(NULL))
        fprintf(stderr, "katzensteg-vulkan-probe-sdl3: SDL_Vulkan_LoadLibrary warning: %s\n", SDL_GetError());

    SDL_WindowFlags flags = SDL_WINDOW_VULKAN | SDL_WINDOW_HIGH_PIXEL_DENSITY | SDL_WINDOW_RESIZABLE;
    app->window = SDL_CreateWindow("katzensteg-vulkan-probe-sdl3",
                                   DEFAULT_WINDOW_WIDTH,
                                   DEFAULT_WINDOW_HEIGHT,
                                   flags);
    if (!app->window) {
        fprintf(stderr, "katzensteg-vulkan-probe-sdl3: SDL_CreateWindow failed: %s\n", SDL_GetError());
        return false;
    }
    if (options->fullscreen_desktop && !SDL_SetWindowFullscreen(app->window, true)) {
        fprintf(stderr, "katzensteg-vulkan-probe-sdl3: SDL_SetWindowFullscreen failed: %s\n", SDL_GetError());
        return false;
    }
...
static void destroy_app(App *app)
{
    ...
    if (app->surface) SDL_Vulkan_DestroySurface(app->instance, app->surface, NULL);
...
int main(int argc, char **argv)
{
    ...
    SDL_GetWindowSizeInPixels(app.window, &drawable_w, &drawable_h);
    fprintf(stderr, "katzensteg-vulkan-probe-sdl3: window=%dx%d drawable=%dx%d swapchain=%ux%u fullscreen_desktop=%s\n",
    ...
    while (running && SDL_GetTicks() - start_ms < run_ms) {
        ...
        if (options.log_frames && (frame % 60u) == 0u)
            fprintf(stderr, "katzensteg-vulkan-probe-sdl3: frame=%u swapchain=%ux%u\n", frame, app.extent.width, app.extent.height);
        frame++;
    }
