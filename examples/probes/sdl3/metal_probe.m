...
int main(int argc, char **argv)
{
    ...
    SDL_WindowFlags flags = SDL_WINDOW_METAL | SDL_WINDOW_HIGH_PIXEL_DENSITY | SDL_WINDOW_RESIZABLE;
    SDL_Window *window = SDL_CreateWindow("katzensteg-metal-probe-sdl3",
    ...
    if (options.fullscreen_desktop && !SDL_SetWindowFullscreen(window, true)) {
    ...
    if (layer == nil || device == nil || queue == nil) {
        fprintf(stderr, "katzensteg-metal-probe-sdl3: Metal setup failed: %s\n", SDL_GetError());
    ...
    fprintf(stderr, "katzensteg-metal-probe-sdl3: window=%dx%d drawable=%dx%d fullscreen_desktop=%s\n",
            window_w,
            window_h,
            drawable_w,
            drawable_h,
    ...
    while (running && SDL_GetTicks() - start_ms < run_ms) {
        ...
        if (!draw_frame(queue, layer, frame)) {
            fprintf(stderr, "katzensteg-metal-probe-sdl3: no drawable available\n");
            break;
        }

        if (options.log_frames && (frame % 60u) == 0u) {
            fprintf(stderr, "katzensteg-metal-probe-sdl3: frame=%u drawable=%dx%d\n", frame, drawable_w, drawable_h);
        }
        frame++;
    }
