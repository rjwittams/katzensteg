...
int main(int argc, char **argv)
{
    ...
    SDL_Window *window = SDL_CreateWindow("katzensteg-gl-probe-sdl3",
    ...
    fprintf(stderr, "katzensteg-gl-probe-sdl3: window=%dx%d drawable=%dx%d fullscreen_desktop=%s\n",
            window_w,
            window_h,
            drawable_w,
            drawable_h,
    ...
    while (running && SDL_GetTicks() - start_ms < run_ms) {
        ...
        if (options.log_frames && (frame % 60u) == 0u) {
            fprintf(stderr, "katzensteg-gl-probe-sdl3: frame=%u drawable=%dx%d\n", frame, drawable_w, drawable_h);
        }
        frame++;
    }
